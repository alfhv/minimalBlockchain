...
def mine():
	...
	# Force nodes to execute Consensus to synch chains
	nodes_synch = ""
	node_synch = ""
	for node in blockchain.nodes:	
		url_request = f'{node}/Node/Consensus'
		node_synch = node
		response = requests.get(url_request)
		if response.status_code == 200:
			node = f'{url_request} notified OK'
		else:
			node = f'{url_request} notified KO: {response.status_code}'
		nodes_synch = f'{nodes_synch} - {node}'
	
	response = {
		'message': "New Block Forged",
		'block': block,
		'node_synch': nodes_synch
	}
...
if __name__ == '__main__':
	...
	url_request = "http://localhost:5001/chain/chain" # f'{node}/node/consensus'
	#response = requests.get(url_request, headers=headers)
	response = requests.get(url_request)
	print(response.text)
	
	if response.status_code == 200:
		print(f'{url_request} notified OK')
	else:
		print(f'{url_request} notified KO: {response.status_code}')
